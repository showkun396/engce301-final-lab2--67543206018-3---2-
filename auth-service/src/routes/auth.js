...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    await logEvent({ level:'INFO', event:'REGISTER_SUCCESS',
      ...
      message: `User ${username} registered`, meta: { username } });
    res.status(201).json({ message: 'Register สำเร็จ', user: result.rows[0] });
  } catch (err) {
    if (err.code === '23505') { // Unique constraint violation
      return res.status(400).json({ error: 'Username หรือ Email นี้มีผู้ใช้งานแล้ว' });
    }
